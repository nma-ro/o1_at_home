...
class Pipe:
    ...
    async def run_reflection(
        ...
        await self.set_status("Thinking about improvements to the response...", __event_emitter__)

        prompt = "You are a reflection model.\n"
        prompt += "Think about the user's query and the response generated by the reasoning model.\n"
        prompt += "Reflect on the response and suggest improvements. Do not rewrite only give additions and what should be included in the answer.\n"
        prompt += f"User Query: {query}\n"
        prompt += f"<response>{response}<response-end>"
        
        reflection = await self.run_step(
            self.valves.RESPONDING_MODEL.strip(), messages, prompt, True, "Reflecting", "Reflection", __event_emitter__
        )
        
        await asyncio.sleep(0.2)
        return reflection
    
    async def run_improving(
        self,
        messages: list,
        query: str,
        response: str,
        ...
        await self.set_status("Improving the response...", __event_emitter__)

        prompt = "You are an improvement model.\n"
        ...
        improvement = await self.run_step(
            self.valves.RESPONDING_MODEL.strip(), messages, prompt, not final_step, "Improving", "Improvement", __event_emitter__
        )
    ...
    async def pipe(
        ...
        if (
            ...
            if self.max_thinking_time_reached:
                ...
                await self.set_status_end(f"Thought for only {thought_duration} seconds", __event_emitter__)
            return ""
        else:
